...
import { GetDisabledPermits, number_format } from "./../../../helpers/helpers";
...
const ListDisabled = (props) => {
  ...
  const openModal = (option, data) => {
    if (option === 1) {
      setOption(option);
      setModalHeader("Registrar divisa");
  ...
  const activar = (data) => {
    const message = {
      title: "Inactivar divisa",
      info: `¿Esta seguro que desea activar la divisa ${data.name}?`,
    };
    confirm(message, (res) => {
      if (res) {
  ...
  return (
    <RctCollapsibleCard heading="Divisas Inactivas" fullBlock>
                    ...
                    ? settingBills.dataDisabled.map((data, i) => {
                      ...
                      return (
                        <TableRow hover key={i}>
                          ...
                          <TableCell style={{ width: "15%" }} align="left">
                            {number_format(data.rate, 2)} {props.currency_symbol}
                          </TableCell>
                          <TableCell
