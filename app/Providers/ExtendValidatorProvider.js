...
class ExtendValidatorProvider extends ServiceProvider {
  ...
  async levelPasswordValidator(data, field, message, args, get) {
    const fieldValue = get(data, field)
    if (
      (fieldValue.match(/^(?=.*[0-9])/) === null)
      || (fieldValue.match(/^(?=.*[A-Za-z])/) === null)
  ...
  async boot() {
    ...
    Validator.extend('levelPassword', this.levelPasswordValidator, '{{field}} is not valid levelPassword')
  }
}
